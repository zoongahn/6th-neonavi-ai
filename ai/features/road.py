...
class PublicIndex:
    ...
    # 서버리스 vCPU 가 로컬보다 ~2.5배 느리다. 로컬 13.8초(24×2km)가 배포에선
    # 다시 60초를 넘겨 한 번 더 조였다 — 24km 분량이면 통계는 충분하다.
    SAMPLE_WINDOWS = 16        # 초과 시: 균등 분포한 창 16개
    SAMPLE_WIN_KM = 1.5        #        × 각 1.5km = 24km 분량만 계산
